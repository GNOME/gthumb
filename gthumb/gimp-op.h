...
#define GIMP_OP_NORMAL(xL, xI, aL)	CLAMP_PIXEL (CAIRO_ADD_ALPHA (xL, aL) + CAIRO_ADD_ALPHA (xI, 255 - aL))
#define GIMP_OP_LIGHTEN_ONLY(xL, xI)	MAX (xI, xL)
#define GIMP_OP_SCREEN(xL, xI)		CLAMP_PIXEL (255 ^ CAIRO_ADD_ALPHA (255 - xI, 255 - xL))
...
#define GIMP_OP_MULTIPLY(xL, xI)	CLAMP_PIXEL (CAIRO_ADD_ALPHA (xL, xI))
#define GIMP_OP_BURN(xL, xI)		CLAMP_PIXEL (255 - GIMP_OP_DIVIDE (xL, 255 - xI))
#define GIMP_OP_SOFT_LIGHT(xL, xI)	CLAMP_PIXEL (CAIRO_ADD_ALPHA (xI, xI) + 2 * CAIRO_ADD_ALPHA (xL, CAIRO_ADD_ALPHA (xI, 255 - xI)))
#define GIMP_OP_HARD_LIGHT(xL, xI)	CLAMP_PIXEL (xL > 128 ? 255 ^ CAIRO_ADD_ALPHA (255 - xI, 2 * (255 - xL)) : CAIRO_ADD_ALPHA (xI, 2 * xL))
