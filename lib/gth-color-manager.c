...
{
	if (data->in_profile != NULL)
		g_object_unref (data->in_profile);
	if (data->out_profile != NULL)
		g_object_unref (data->out_profile);
	if (data->transform != NULL)
		g_object_unref (data->transform);
	g_free (data);
}
...
gth_color_manager_finalize (GObject *object)
{
	...
	if (self->priv->cd_client != NULL)
		g_object_unref (self->priv->cd_client);
...
{
	if (data->color_manager != NULL)
		g_object_unref (data->color_manager);
	if (data->icc_file != NULL)
		g_object_unref (data->icc_file);
...
static void
profile_buffer_ready_cb (GObject	*source_object,
			 GAsyncResult	*res,
			 gpointer	 user_data)
{
	GTask *task = user_data;
	GError *error = NULL;
	GBytes *bytes = g_file_load_bytes_finish (G_FILE (source_object), res, NULL, &error);
			...
			gpointer	 user_data)
{
	...
	GPtrArray *devices = cd_client_get_devices_by_kind_finish (cd_client, res, NULL);
	...
	CdDevice *device = g_object_ref (g_ptr_array_index (devices, 0));
			       ...
			       gpointer		 user_data)
{
	...
	CdDevice *device = cd_client_find_device_by_property_finish (cd_client, res, NULL);
	if (device == NULL) {
				...
				const char	*id)
{
	...
	GthIccProfile *profile = g_hash_table_lookup (self->priv->profile_cache, id);
	return (profile != NULL) ? g_object_ref (profile) : NULL;
}
