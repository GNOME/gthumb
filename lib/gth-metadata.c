...
gth_metadata_get_string_list (GthMetadata *metadata)
{
	...
	else if (metadata->priv->data_type == GTH_METADATA_TYPE_STRING_LIST) {
...
static GPtrArray *metadata_info_array;
...
GthMetadataCategory * gth_metadata_category_get (const char *id) {
	return (id != NULL) ? g_hash_table_lookup (metadata_category_hash, id) : NULL;
}
...
void gth_metadata_category_register (const char *id, const char *display_name) {
	...
	g_mutex_unlock (&metadata_category_mutex);
}


GthMetadataCategory * gth_metadata_category_new (const char *id, const char *display_name, int sort_order) {
	GthMetadataCategory *new_category = g_new0 (GthMetadataCategory, 1);
	if (id != NULL)
		new_category->id = g_strdup (id);
	if (display_name != NULL)
		new_category->display_name = g_strdup (display_name);
	new_category->sort_order = sort_order;
	return new_category;
}


GthMetadataCategory * gth_metadata_category_copy (GthMetadataCategory *category) {
	return gth_metadata_category_new (
		category->id,
