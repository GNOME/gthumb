...
typedef struct {
	...
	GtkWidget    *dialog;
	GtkWidget    *bc_hue_hscale;
	GtkWidget    *bc_lightness_hscale;
	GtkWidget    *bc_saturation_hscale;
	       ...
	       gboolean    preview)
{
	...
	_gdk_pixbuf_hue_lightness_saturation (src,
					      dest,
					      hue,
					      lightness,
					      saturation);
	  ...
	  DialogData *data)
{
	...
	gtk_range_set_value (GTK_RANGE (data->bc_lightness_hscale), 0.0);
	gtk_range_set_value (GTK_RANGE (data->bc_saturation_hscale), 0.0);
}
...
void
dlg_hue_saturation (GThumbWindow *window)
{
	DialogData *data;
	GtkWidget  *ok_button;
